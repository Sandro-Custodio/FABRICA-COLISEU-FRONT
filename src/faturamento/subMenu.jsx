...
const SubMenu = props => {
  ...
  const isSubMenu = action => {
    return action.code.includes("DR_COF1") && action.nivel === "SUBMENU_1";
  };

  const isDR_COF1 = action => {
    return action.code === "DR_COF1";
  };

  const permissionsFilteredSubMenu = auth.user.permissions.filter(isSubMenu);

  const allow = auth.user.permissions.filter(isDR_COF1).length === 1;

  return (
    <div className="fade-in fade-out">
      <Acl check={allow}>
        <HeaderContent title="Faturamento" small="Menu" />
        <Content>
