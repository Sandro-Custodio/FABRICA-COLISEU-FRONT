...
const SubMenu = props => {
  ...
  const isSubMenu = action => {
    return ((action.code.includes("DR_COE1"))||(action.code.includes("DR_COE1D1"))) && action.nivel === "SUBMENU_1";
  };

  const isDR_COE1 = action => {
    return action.code === "DR_COE1";
  };

  const permissionsFilteredSubMenu = auth.user.permissions.filter(isSubMenu);

  const allow = auth.user.permissions.filter(isDR_COE1).length === 1;

  return (
    <div className="fade-in fade-out">
      <Acl check={allow}>
        <HeaderContent title="SDs" small="Menu" />
        <Content>
