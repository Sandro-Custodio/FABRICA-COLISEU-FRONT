...
import { saveNewDemand, get_demand_classifications } from "./actions";
...
class DemandComp extends Component {
    ...
    submitForm = async (values) => {
       ...
       /* saveNewDemand(values).then(resp => {
            console.log(resp)
            if (resp.status === 200) {
                get_demand_classifications()
            } else {
        ...
        try {
           await saveNewDemand(values)
           await get_demand_classifications()
        } catch (error) {
            console.log(error);
        }
    };
        ...
        return (
            <React.Fragment>
                <button
                    type="button"
                    data-toggle="modal"
                    data-target="#demanda-de-classificacao"
