...
const SubMenu = props => {
  ...
  const isSubMenu = action => {
    return action.code.includes("DR_COB1") && action.nivel === "SUBMENU_1";
  };

  const isDR_COB1 = action => {
    return action.code === "DR_COB1";
  };

  const permissionsFilteredSubMenu = auth.user.permissions.filter(isSubMenu);

  const allow = auth.user.permissions.filter(isDR_COB1).length === 1;

  return (
    <div className="fade-in fade-out">
      <Acl check={allow}>
        <HeaderContent title="EVT" small="Menu" />
        <Content>
