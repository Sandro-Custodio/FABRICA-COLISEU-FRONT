...
export const get_all_credit_prev = ({ page, limit }) => {
  return async dispatch => {
    dispatch({ type: "SHOW_OVERLAY" });
    await axios
      .post("/bill_reports/get_all_credit_prev", { page, limit })
      .then(res =>
        dispatch([
          { type: "GET_ALL_CREDIT_PREV", payload: res.data },
          { type: "HIDE_OVERLAY" }
        ])
      )
...
export const export_all = (columns, data, access_token) => {
  return async dispatch => {
    dispatch({ type: "SHOW_OVERLAY" });
    await axios
      .post("/bill_reports/send_bill_report_credit_foresight", { code: "DR_COF1J1J1", no_limit: true })
      .then(res => {
