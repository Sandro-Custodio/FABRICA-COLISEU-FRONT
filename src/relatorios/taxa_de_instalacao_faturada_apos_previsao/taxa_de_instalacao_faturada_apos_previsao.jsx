...
const TaxaDeInstalacaoFaturadaAposPrevisao = props => {
    ...
    const mapAndFix = rows => rows.map(i => ({
        ...
        taxa_ins_fat_s_imp: i.taxa_ins_fat_s_imp && fix(i.taxa_ins_fat_s_imp),
        taxa_ins_fat_c_imp: i.taxa_ins_fat_s_imp && fix(i.taxa_ins_fat_s_imp),
    }))
    ...
    const handle_change_page = e => {
        if (!fired) {
            ...
            if (e.key === 'Enter') {
                ...
                switch (go_page) {
                    case "first":
                        return get_post_fat({ page: 1, limit })
                    case "target":
                        return get_post_fat({ page, limit })
                    case "last":
                        return get_post_fat({ page: pages, limit })
                }
            }
        }
    }
    ...
    const backward_or_forward = new_page => {
        if (new_page > 0 && new_page <= pages) {
            set_page(new_page)
            get_post_fat({ page: new_page, limit })
        }
    }
            ...
            <div className="fade-in fade-out">
                <div className="header">
                    <div className="header__left-items">
                        <ContentHeader title="Taxa de Instalação Faturada Após Previsão" />
                    </div>
                </div>
                <Content>
                    <Grid cols="12">
                        <DxGrid rows={list_table} columns={columns}>
                            ...
                            <ExportCsv
                                ...
                                name="Taxa de Instalação Faturada Após Previsão"
                            />
...
const mapStateToProps = state => ({
    auth: state.auth,
    reducer: state.taxaDeInstalacaoFaturadaAposPrevisaoReducer,
...
export default connect(
    mapStateToProps,
    mapDispatchToProps
)(TaxaDeInstalacaoFaturadaAposPrevisao)