...
const FaturasInvalidas = props => {
  ...
  const list = rows.map(i => ({
    ...
    emissao:
      i.emissao &&
      i.emissao !== "" && new Date(i.emissao).toLocaleDateString("pt-BR"),
    vencimento:
      i.vencimento &&
  ...
  const handle_filter = (page = 1, use_state = false) => {
    ...
    if (ok || use_state) {
      ...
      get_all_faturas_invalidas(request).then(_ => !use_state && set_page(1));
      if (!use_state) {
  ...
  const formatar_fatura = ({ value }) => (
    <a style={{ color: "#333", textDecoration: "underline" }}>
      <b>{value}</b>
    </a>
  );

  const FaturaProvider = props => (
    <DataTypeProvider formatterComponent={formatar_fatura} {...props} />
  );
      ...
      <div className="fade-in fade-out">
        <div className="header">
          <div className="header__left-items">
            <ContentHeader title="Faturas Inválidas" />
          </div>
        </div>
        <Content>
          <Grid cols="12">
          ...
          <Grid cols="12">
            <DxGrid rows={list} columns={columns}>
              ...
              <ExportCsv
                rows={rows}
                columns={columns}
                name="Faturas Inválidas"
                cached_params={cached_params}
                data_export={data_export}
              />
              <FaturaProvider for={["bill_number"]} />
...
const form = reduxForm({ form: "faturasInvalidas" })(FaturasInvalidas);

const mapStateToProps = state => ({
  auth: state.auth,
  pg_form: state.form.faturasInvalidas,
  reducer: state.faturasInvalidasReducer,
