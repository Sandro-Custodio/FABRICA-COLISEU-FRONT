...
export const get_not_fat = ({ page, limit }) => {
  return async dispatch => {
    dispatch({ type: "SHOW_OVERLAY" });
    await axios
      .post("/bill_reports/get_not_fat", { page, limit })
      .then(res =>
        dispatch([
          { type: "GET_NOT_FAT", payload: res.data },
          { type: "HIDE_OVERLAY" }
        ])
      )
...
export const export_all = ({ access_token }) => {
  return async dispatch => {
    dispatch({ type: "SHOW_OVERLAY" });
    await axios
      .post("/bill_reports/send_not_fat_report", { code: "DR_COF1J1K1", no_limit: true })
      .then(res => {
