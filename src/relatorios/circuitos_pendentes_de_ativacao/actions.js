...
export const get_all_bill_foresights = ({ page, limit }) => {
  return async dispatch => {
    dispatch({ type: "SHOW_OVERLAY" });
    await axios
      .post("/bill_reports/get_all_bill_foresights", { page, limit })
      .then(res =>
        dispatch([
          { type: "GET_ALL_BILL_FORESIGHTS", payload: res.data },
          { type: "HIDE_OVERLAY" }
        ])
      )
...
export const export_all = (access_token) => {
  return async dispatch => {
    dispatch({ type: "SHOW_OVERLAY" });
    await axios
      .post("/bill_reports/send_bill_report_foresights", {code: "DR_COF1J1H1", no_limit: true })
      .then(res => {
