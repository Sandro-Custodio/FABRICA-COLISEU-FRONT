...
import {
    get_all_bill_foresights,
} from "./actions"

const CircuitosPendentesDeAtivacao = props => {
    ...
    React.useEffect(_ => {
        get_all_bill_foresights({ page: 1, limit })
        return undefined
    }, [])
    ...
    const mapAndFix = rows => rows.map(i => ({
        ...
        data_report: i.data_report && fixDate(i.data_report),
    ...
    const handle_change_page = e => {
        if (!fired) {
            ...
            if (e.key === 'Enter') {
                ...
                switch (go_page) {
                    case "first":
                        return get_all_bill_foresights({ page: 1, limit })
                    case "target":
                        return get_all_bill_foresights({ page, limit })
                    case "last":
                        return get_all_bill_foresights({ page: pages, limit })
                }
            }
        }
    }
    ...
    const backward_or_forward = new_page => {
        if (new_page > 0 && new_page <= pages) {
            set_page(new_page)
            get_all_bill_foresights({ page: new_page, limit })
        }
    }
            ...
            <div className="fade-in fade-out">
                <div className="header">
                    <div className="header__left-items">
                        <ContentHeader title="Circuitos Pendentes de Ativação" small="Relatórios de Créditos Previstos" />
                    </div>
                </div>
                <Content>
                    <Grid cols="12">
                        <DxGrid rows={list} columns={columns}>
                            ...
                            <ExportCsv
                                rows={list}
                                columns={columns}
                                name="Circuitos Pendentes de Ativação"
...
const mapStateToProps = state => ({
    auth: state.auth,
    reducer: state.circuitosPendentesDeAtivacaoReducer,
