...
const SubMenu = props => {
  ...
  const isSubMenu = action => {
    return action.code.includes("DR_COP1") && action.nivel === "SUBMENU_1";
  };

  const isDR_COP1 = action => {
    return action.code === "DR_COP1";
  };

  const permissionsFilteredSubMenu = auth.user.permissions.filter(isSubMenu);

  const allow = auth.user.permissions.filter(isDR_COP1).length === 1;

  return (
    <div className="fade-in fade-out">
      <Acl check={allow}>
        <HeaderContent title="Acesso" small="Menu" />
        <Content>
