...
const SubMenu = props => {
  ...
  const isSubMenu = action => {
    return action.code.includes("DR_COC1") && action.nivel === "SUBMENU_1";
  };

  const isDR_COC1 = action => {
    return action.code === "DR_COC1";
  };

  const permissionsFilteredSubMenu = auth.user.permissions.filter(isSubMenu);

  const allow = auth.user.permissions.filter(isDR_COC1).length === 1;

  return (
    <div className="fade-in fade-out">
      <Acl check={allow}>
        <HeaderContent title="Leasedlines" small="Menu" />
        <Content>
